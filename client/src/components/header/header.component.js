...
import {MobileLogo, MobilePlayContainer, MobileStyledHeader, MobilePlaySecondText, MobilePlayFirstText} from "./mobileHeader.styles";
            ...
            <MobileView>
                <MobileStyledHeader>
                    ...
                    <MobilePlayContainer>
                        ...
                        <MobilePlayFirstText>{user ? 'Music on pause' : 'Log in to track'}</MobilePlayFirstText>
                    </MobilePlayContainer>
                    {user && user.avatarUrl ?
                        <UserAvatar onClick={() => {navigate("/account")}}
                                    src={user.avatarUrl} alt="User avatar"/>
                        :
                        <SpotifyLogo onClick={()=>{window.location.href = 'http://192.168.1.98:3000/auth/request-authorization'}}
